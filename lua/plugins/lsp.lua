	...
	config = function()
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(event)
				...
				map("n", "gr", "<cmd>Telescope lsp_references initial_mode=normal<cr>", "Goto References")
				map("n", "gm", "<cmd>Telescope lsp_document_symbols initial_mode=normal<cr>", "Goto References")
					...
					["textDocument/publishDiagnostics"] = function(err, result, ctx, config)
						...
						result.diagnostics = vim.tbl_filter(function(diagnostic)
							...
							if string.match(result.uri, "/[vV]i[es][wt][as]?/") then
								...
								if string.match(diagnostic.message, "Undefined variable") then
									return false
								end
							end
